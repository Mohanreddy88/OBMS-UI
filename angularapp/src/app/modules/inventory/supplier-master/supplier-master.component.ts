...
export class SupplierMasterComponent implements AfterViewInit {
      ...
      (data) => {
        if (data != null) {
          ...
          if (this.userAccessModel.readAccess === true || this.currentUser == 'superadmin') {
            this.warningMessage = '';
            this.service.getSuppliers().subscribe((d: any) => {
              if (d.length > 0) {
      ...
      .subscribe((result: { confirmDialog: boolean; remarks: any }) => {
        if (result.confirmDialog) {
          ...
          this.service.deleteSupplierType(id).subscribe({
            next: res => {
              this.showMessage(`Supplier deleted successfully.`, 'success', 'Success Message');
              this.route.navigateByUrl('/dummy', { skipLocationChange: true }).then(() => {
                this.route.navigate(['/inventory/supplier-master']);
              });
            },
            error: err => {
              this.showMessage(`Supplier Failed to delete records due to ${err}`, 'error', 'Error Message');
            }
          });
