...
export class BranchTransactionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getUserAccessRights(this.currentUser, 'Branch Transactions Report');
  }
  ...
  clkBtn(number: number) {    
    this.reportPageName = number == 1 ? "BranchTransactionReport.aspx?" : 'InvoiceCollectionTotalReportNoTax.aspx?';
